...
import cleancode.studycafe.tobe2.model.Calculate;
import cleancode.studycafe.tobe2.model.StudyCafeLockerPass;
import cleancode.studycafe.tobe2.model.StudyCafePass;

import java.util.List;

public class KioskOutputHandler implements OutputHandler{


    @Override
    public void showWelcomeAndManualMessage() {
        showWelcomeMessage();
        showAnnouncement();
    ...
    public void showPassOrderSummary(StudyCafePass selectedPass, StudyCafeLockerPass lockerPass) {
        ...
        int discountPrice = calculate.discountCalculate(selectedPass);
        if (discountPrice > 0) {
            System.out.println("이벤트 할인 금액: " + discountPrice + "원");
        }

        int totalPrice = calculate.totalCalculate(selectedPass, lockerPass);
